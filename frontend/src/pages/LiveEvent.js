...
const SIGNAL_CATEGORIES = [
  {
    ...
    signals: [
      { value: "coach_interest", label: "Coach Interest", icon: Eye, color: "text-red-400", bg: "bg-red-500/10", glow: "shadow-red-500/20", consequence: "Follow-up within 48h", suggest: "Send highlight + thank you message" },
      { value: "offered_visit", label: "Offered Visit", icon: ArrowRight, color: "text-emerald-400", bg: "bg-emerald-500/10", glow: "shadow-emerald-500/20", consequence: "Pipeline advances to Campus Visit", suggest: "Confirm visit date and logistics" },
    ],
  },
  {
    key: "performance", title: "Performance", icon: Star,
    signals: [
      { value: "strong_performance", label: "Strong Performance", icon: Star, color: "text-amber-400", bg: "bg-amber-500/10", glow: "shadow-amber-500/20", consequence: "Boosts athlete profile visibility", suggest: "Capture specific drill or play details" },
      { value: "standout_skill", label: "Standout Skill", icon: Zap, color: "text-yellow-400", bg: "bg-yellow-500/10", glow: "shadow-yellow-500/20", consequence: "Highlights unique ability for coaches", suggest: "Note the specific skill observed" },
    ],
  },
  {
    key: "needs", title: "Needs / Risks", icon: AlertTriangle,
    signals: [
      { value: "needs_film", label: "Needs Film", icon: Film, color: "text-sky-400", bg: "bg-sky-500/10", glow: "shadow-sky-500/20", consequence: "Momentum drops if not sent within 72h", suggest: "Request highlight reel from athlete" },
    ],
  },
  {
    key: "milestones", title: "Milestones", icon: Trophy,
    signals: [
      { value: "good_conversation", label: "Good Conversation", icon: MessageSquare, color: "text-violet-400", bg: "bg-violet-500/10", glow: "shadow-violet-500/20", consequence: "Logs positive engagement milestone", suggest: "Send thank-you follow-up" },
    ],
  },
];

const ALL_SIGNALS = SIGNAL_CATEGORIES.flatMap(c => c.signals);
function getSignalDef(value) { return ALL_SIGNALS.find(s => s.value === value); }
...
/* ─── Live Impact Panel ─── */
function LiveImpactPanel({ selectedAthletes, school, interest, signalType, athletes, schools }) {
  const sig = signalType ? getSignalDef(signalType) : null;
  const athNames = selectedAthletes.map(id => athletes.find(a => a.id === id)?.full_name?.split(" ")[0]).filter(Boolean);
  const schlName = school ? (schools.find(s => s.id === school)?.name || "") : null;

  if (selectedAthletes.length === 0 && !signalType) return null;
  ...
  if (sig) {
    ...
    if (isHighPriority) {
      icon = Flame; iconColor = "text-red-400"; borderColor = "border-red-500/20"; bgColor = "bg-red-950/30";
    } else if (isRisk) {
      icon = AlertTriangle; iconColor = "text-amber-400"; borderColor = "border-amber-500/20"; bgColor = "bg-amber-950/30";
    } else {
      icon = Check; iconColor = "text-emerald-400"; borderColor = "border-emerald-500/20"; bgColor = "bg-emerald-950/30";
    }

    const target = schlName || "this school";
    if (isHighPriority) {
      lines.push(`${target} → ${sig.consequence.toLowerCase()}`);
    } else {
      lines.push(sig.consequence);
    }
    if (sig.suggest) lines.push(sig.suggest);
  } else if (selectedAthletes.length > 0 && !signalType) {
    lines.push("Select a signal to see what happens next");
  }

  if (selectedAthletes.length > 1) {
    lines.push(`Applies to ${athNames.join(", ")}`);
  }
  ...
  return (
    <div
      ...
      className={`${bgColor} border ${borderColor} rounded-xl px-3.5 py-2.5 transition-all duration-200`}
    ...
    >
      <div className="flex items-start gap-2.5">
        ...
        <div className="flex-1 min-w-0">
          {lines.map((line, i) => (
            <p key={i} className={`text-[11px] leading-relaxed ${i === 0 ? "text-gray-200 font-semibold" : "text-gray-400"}`}>
...
function LiveEvent() {
  ...
  const [lastSavedIds, setLastSavedIds] = useState([]);
  ...
  // Union of target schools for all selected athletes
  const sortedSchools = useMemo(() => {
    if (selectedAthletes.length === 0) return schools.slice(0, 12);
    const allTargets = new Set();
    for (const id of selectedAthletes) {
      const ath = athletes.find(a => a.id === id);
      (ath?.targetSchoolsAtEvent || []).forEach(n => allTargets.add(n.toLowerCase()));
    }
    const isTarget = (s) => [...allTargets].some(t => s.name.toLowerCase().includes(t) || t.includes(s.name.toLowerCase()));
    return [...schools.filter(s => isTarget(s)), ...schools.filter(s => !isTarget(s))].slice(0, 12);
  }, [selectedAthletes, athletes, schools]);
  ...
  const targetSchoolIds = useMemo(() => {
    if (selectedAthletes.length === 0) return new Set();
    const allTargets = new Set();
    for (const id of selectedAthletes) {
      const ath = athletes.find(a => a.id === id);
      (ath?.targetSchoolsAtEvent || []).forEach(n => allTargets.add(n.toLowerCase()));
    }
    return new Set(schools.filter(s => [...allTargets].some(t => s.name.toLowerCase().includes(t) || t.includes(s.name.toLowerCase()))).map(s => s.id));
  }, [selectedAthletes, athletes, schools]);
  ...
  const recentSchoolIds = useMemo(() => {
    ...
    const athNotes = notes.filter(n => selectedAthletes.includes(n.athlete_id));
    const seen = new Set();
    const ids = [];
  ...
  const filteredGroups = useMemo(() => {
    return selectedAthletes.length > 0 && !showAllSignals
      ? groupedNotes.filter(g => selectedAthletes.includes(g.athleteId))
      : groupedNotes;
  }, [groupedNotes, selectedAthletes, showAllSignals]);
  ...
  const logSignal = async () => {
    if (selectedAthletes.length === 0) { toast.error("Select an athlete"); return; }
    if (!signalType) { toast.error("Select a signal type"); return; }
    if (!noteText.trim()) { toast.error("Add a note"); return; }
    setSaving(true);
    try {
      ...
      setTimeout(() => setLastSavedIds([]), 3000);

      const names = selectedAthletes.map(id => athletes.find(a => a.id === id)?.full_name?.split(" ")[0]).filter(Boolean);
      let msg = names.length === 1 ? names[0] : `${names.length} athletes`;
      if (school?.name) msg += ` + ${school.name}`;
      if (newNotes.some(n => n.action_created)) msg += " · Actions created";
      toast.success(msg);
  ...
  const addSchool = async () => {
    if (selectedAthletes.length === 0 || !newSchoolName.trim()) return;
  ...
  const canSave = selectedAthletes.length > 0 && signalType && noteText.trim();
  ...
  const signalsPanel = (
    <div data-testid="recent-signals-feed">
      ...
      <div className="space-y-2 max-h-[50vh] lg:max-h-[60vh] overflow-y-auto pr-1 custom-scrollbar">
        {filteredGroups.length === 0 ? (
          <p className="text-xs text-gray-500 text-center py-8">
            {selectedAthletes.length > 0 && !showAllSignals ? "No signals for selected athletes yet." : "No signals yet. Start capturing."}
          </p>
        ) : (
          filteredGroups.map((g, gi) => (
            <div key={gi} className="bg-gray-900/80 border border-gray-800/60 rounded-lg overflow-hidden" data-testid={`signal-group-${gi}`}>
              <div className="px-3 py-1.5 bg-gray-900/90 border-b border-gray-800/50 flex items-center gap-2">
                <span className="text-[11px] font-semibold text-white">{g.athleteName?.split(" ")[0]}</span>
                {g.schoolName && (
                  <>
                    <span className="text-gray-600 text-[10px]">&mdash;</span>
                    <span className="text-[11px] text-gray-300 truncate">{g.schoolName}</span>
                  </>
                )}
                <span className="text-[9px] text-gray-500 ml-auto shrink-0">{g.signals.length}</span>
              </div>
              <div className="divide-y divide-gray-800/30">
                {g.signals.map((n) => {
                  ...
                  const isJustSaved = lastSavedIds.includes(n.id);
                  return (
                    <div key={n.id} className={`px-3 py-1.5 transition-all duration-300 ${isJustSaved ? "bg-emerald-900/15" : ""}`} data-testid={`recent-signal-${n.id}`}>
                      <div className="flex items-center gap-1.5 flex-wrap">
                        {SigIcon && <SigIcon className={`w-3 h-3 shrink-0 ${sigDef.color}`} />}
                        {sigDef && <span className="text-[10px] font-medium text-gray-200">{sigDef.label}</span>}
                        {intStyle && n.interest_level !== "none" && (
                          <span className={`text-[9px] px-1.5 py-0.5 rounded font-bold ${intStyle.cls}`}>{intStyle.label}</span>
                        )}
                        {n.pipeline_updated && (
                          <span className="text-[8px] px-1 py-0.5 rounded bg-emerald-900/30 text-emerald-400 font-medium">Pipeline</span>
                        )}
                        {isJustSaved && (
                        ...
                        <span className="text-[9px] text-gray-500 ml-auto shrink-0 flex items-center gap-0.5">
                          <Clock className="w-2.5 h-2.5" /> {timeAgo(n.captured_at)}
                        </span>
                      </div>
                      {n.note_text && <p className="text-[10px] text-gray-400 mt-0.5 line-clamp-2">{n.note_text}</p>}
                    </div>
                  );
                })}
              </div>
  ...
  const captureForm = (
    <div className="space-y-4">
      ...
      {/* ═══ ATHLETES — Horizontal cards, multi-select ═══ */}
      <section>
        <div className="flex items-center gap-2 mb-2">
          <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider">Athletes</label>
          {selectedAthletes.length > 1 && (
            <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-white/10 text-white font-bold" data-testid="multi-select-count">
              {selectedAthletes.length} selected
            </span>
          )}
        </div>
        <div className="flex gap-2 overflow-x-auto pb-1 -mx-1 px-1 snap-x" data-testid="athlete-cards">
          {athletes.map((a) => {
            ...
            return (
              <button key={a.id}
                ...
                className={`flex-shrink-0 snap-start flex items-center gap-2.5 pl-2 pr-4 py-2 rounded-2xl transition-all duration-200 relative min-w-[120px] ${
                  isSelected
                    ? "bg-white text-gray-900 shadow-lg shadow-white/10 scale-[1.02]"
                    : "bg-gray-800/80 text-gray-300 hover:bg-gray-750 border border-gray-700/40"
                }`}
                style={isSelected ? { boxShadow: "0 0 16px rgba(255,255,255,0.08)" } : undefined}
              >
                <div className="relative">
                  {a.photo_url ? (
                    ...
                    <div className={`w-9 h-9 rounded-full flex items-center justify-center text-[11px] font-bold ${
                      isSelected ? "bg-gray-200 text-gray-700" : "bg-gray-600 text-white"
                    }`}>
                  ...
                  } ${covered ? "bg-emerald-400" : "bg-gray-500"}`} />
                </div>
                <span className="font-semibold text-[12px] truncate">{a.full_name?.split(" ")[0]}</span>
                {isSelected && (
                  <Check className="w-3.5 h-3.5 absolute top-1.5 right-1.5 text-gray-400" />
                )}
              </button>
            );
          })}
      ...
      <section>
        <div className="flex items-center gap-2 mb-1.5">
          ...
          {selectedAthletes.length > 0 && targetSchoolIds.size > 0 && (
        ...
        <div className="relative mb-1.5">
          ...
          <input
            ...
            className="w-full bg-gray-800/50 border border-gray-700/40 rounded-xl pl-8 pr-3 py-2 text-xs text-white placeholder-gray-500 focus:outline-none focus:border-gray-500 transition-colors"
          />
        </div>

        {/* Recent schools */}
        {recentSchoolIds.length > 0 && !schoolSearch && (
          <div className="flex items-center gap-1.5 mb-1.5 flex-wrap" data-testid="recent-schools">
        ...
        <div className="flex flex-wrap gap-1.5" data-testid="school-chips">
          {filteredSchools.map((s) => {
            ...
            return (
              <button key={s.id}
                ...
                className={`px-3 py-2 rounded-xl text-[11px] font-medium transition-all duration-150 active:scale-95 min-h-[38px] ${
                  selectedSchool === s.id
                    ? "bg-white text-gray-900 ring-2 ring-white/20 shadow-md shadow-white/5"
                    : isTarget
                      ? "bg-emerald-900/25 text-emerald-300 border border-emerald-600/25 hover:bg-emerald-800/35"
                      : "bg-gray-800/60 text-gray-300 border border-gray-700/40 hover:bg-gray-700/60"
                }`}
              >
                {s.name}
              </button>
          ...
          {!showAddSchool && (
            <button
              ...
              className="px-3 py-2 rounded-xl text-[11px] font-semibold border-2 border-dashed border-gray-600 text-gray-400 hover:text-white hover:border-gray-400 transition-all duration-150 active:scale-95 flex items-center gap-1.5 min-h-[38px]"
            >
      ...
      {/* ═══ INTEREST ═══ */}
      <section>
        <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-1.5 block">Interest</label>
        <div className="grid grid-cols-4 gap-1.5" data-testid="interest-selector">
          {INTEREST.map((i) => (
            <button key={i.value}
              ...
              className={`px-2 py-2 rounded-xl text-[11px] font-bold transition-all duration-150 active:scale-95 text-center min-h-[40px] ${
                interest === i.value ? `${i.cls} ring-2 ${i.ring}` : "bg-gray-800/60 text-gray-400 border border-gray-700/40 hover:bg-gray-700/60"
              }`}
            >
              {i.label}
            </button>
        ...
        {interest && (
          <p data-testid="interest-guidance" className="text-[10px] text-gray-400 mt-1 pl-0.5"
            style={{ animation: "impactIn 120ms ease both" }}>
            {INTEREST.find(i => i.value === interest)?.guide}
          </p>
        )}
      </section>

      {/* ═══ SIGNALS — Grouped with glow expand ═══ */}
      <section>
        <label className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mb-2 block">Signal</label>
        <div className="space-y-2.5" data-testid="signal-categories">
            ...
            return (
              <div key={cat.key} data-testid={`signal-cat-${cat.key}`}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-[9px] font-semibold text-gray-500 uppercase tracking-wider">{cat.title}</span>
                </div>
                    ...
                    return (
                      <div key={s.value} className="flex flex-col">
                        <button
                          ...
                          className={`flex items-center gap-2 px-3 py-2.5 rounded-xl text-left transition-all duration-200 border min-h-[42px] ${
                            selected
                              ? `${s.bg} border-transparent text-white shadow-lg ${s.glow}`
                              : "bg-gray-800/40 border-gray-700/30 text-gray-400 hover:border-gray-600 hover:text-gray-300 active:scale-95"
                          }`}
                          style={selected ? { boxShadow: `0 0 20px var(--glow-color, rgba(255,255,255,0.06))` } : undefined}
                        >
                          <Icon className={`w-4 h-4 shrink-0 transition-colors duration-200 ${selected ? s.color : "text-gray-500"}`} />
                          <span className={`text-[11px] font-semibold truncate ${selected ? "text-white" : ""}`}>{s.label}</span>
                        </button>
                        {/* Inline expand: consequence + suggest */}
                        {selected && (
                          <div className="px-3 py-2 mt-1 rounded-lg bg-gray-800/40 border border-gray-700/20" style={{ animation: "expandIn 180ms ease both" }} data-testid="signal-intelligence">
                            <p className="text-[10px] text-gray-200 font-semibold flex items-center gap-1">
                              <Flame className="w-3 h-3 text-gray-400 shrink-0" />
                              {s.consequence}
                            </p>
                            {s.suggest && (
      ...
      <section>
        ...
        <textarea
          ...
          placeholder="What did the coach say or do?"
          maxLength={280}
          rows={2}
          data-testid="note-text"
          className="w-full bg-gray-800/40 border border-gray-700/30 rounded-xl px-3 py-2.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-gray-500 resize-none transition-colors"
        />
      </section>

      {/* ═══ CTA ═══ */}
      <div className="space-y-2.5" data-testid="capture-cta">
        <label className="flex items-center gap-2.5 cursor-pointer group" data-testid="notify-athlete-toggle">
          <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all duration-150 ${
            notifyAthlete ? "bg-emerald-500 border-emerald-500" : "border-gray-600 group-hover:border-gray-400"
          }`} onClick={() => setNotifyAthlete(!notifyAthlete)}>
            {notifyAthlete && <Check className="w-3 h-3 text-white" />}
          </div>
          <span className="text-[11px] text-gray-400 font-medium flex items-center gap-1.5" onClick={() => setNotifyAthlete(!notifyAthlete)}>
            {notifyAthlete ? <Bell className="w-3.5 h-3.5 text-emerald-400" /> : <BellOff className="w-3.5 h-3.5" />}
            Notify athlete{selectedAthletes.length > 1 ? "s" : ""}
          </span>
        </label>
        ...
        <button
          ...
          className={`w-full py-3 font-bold text-sm rounded-xl transition-all duration-200 active:scale-[0.98] flex items-center justify-center gap-2 min-h-[48px] ${
            canSave
        ...
        >
          {saving ? (
            ...
            <>
              ...
              {selectedAthletes.length > 1 && <span className="text-[10px] font-normal opacity-60">({selectedAthletes.length})</span>}
            </>
          )}
        </button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white" data-testid="live-event-page">
      ...
      {/* Header */}
      <header className="bg-gray-950/95 border-b border-gray-800/50 backdrop-blur-sm sticky top-0 z-50">
      ...
      {/* Compact Stats Bar */}
      <div className="bg-gray-900/40 border-b border-gray-800/30" data-testid="live-stats-bar">
        <div className="max-w-[960px] mx-auto px-3 sm:px-4 py-1.5 flex items-center justify-between">
          <div className="flex items-center gap-3 sm:gap-4">
            <div className="flex items-center gap-1.5 text-[10px]">
              <FileText className="w-3 h-3 text-gray-500" />
              <span className="font-semibold text-gray-200" data-testid="stat-notes-count">{notes.length}</span>
              <span className="text-gray-500">signals</span>
            </div>
            <div className="w-px h-3 bg-gray-800" />
            <div className="flex items-center gap-1.5 text-[10px]">
              <Users className="w-3 h-3 text-gray-500" />
              <span className="font-semibold text-gray-200" data-testid="stat-athletes-covered">{athletesCovered.size}/{athletes.length}</span>
              <span className="text-gray-500 hidden sm:inline">athletes</span>
            </div>
          </div>
          <div className="text-[9px] text-gray-500 hidden md:block">
            <kbd className="px-1 py-0.5 bg-gray-800 rounded text-gray-400 font-mono">Ctrl+Enter</kbd> save
          </div>
        </div>
      </div>

      {/* Mobile Tab Toggle */}
      <div className="lg:hidden bg-gray-950 border-b border-gray-800/30">
        <div className="max-w-[960px] mx-auto px-3 sm:px-4 flex">
          <button onClick={() => setMobileTab("capture")} data-testid="tab-capture"
            className={`flex-1 py-2 text-[11px] font-semibold text-center transition-colors border-b-2 ${
          ...
          <button onClick={() => setMobileTab("signals")} data-testid="tab-signals"
            className={`flex-1 py-2 text-[11px] font-semibold text-center transition-colors border-b-2 flex items-center justify-center gap-1.5 ${
      ...
      <div className="max-w-[960px] mx-auto px-3 sm:px-4 py-3 pb-8">
      ...
      <style>{`
        ...
        @keyframes expandIn { from { opacity: 0; transform: scaleY(0.8); transform-origin: top } to { opacity: 1; transform: scaleY(1) } }
