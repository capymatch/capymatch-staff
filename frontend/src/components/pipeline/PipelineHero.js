...
export default function PipelineHero({ heroItems, matchScores, navigate }) {
  ...
  const label = lvl === "high"
    ? { text: "Needs attention", bg: "rgba(255,107,127,0.14)", color: "#ffc4cf" }
    : lvl === "medium"
      ? { text: "Gaining momentum", bg: "rgba(255,155,82,0.14)", color: "#ffd29f" }
    ...
    >
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20, position: "relative", zIndex: 1 }}>
        <span data-testid="hero-category-label" style={{
          ...
          padding: "6px 12px", borderRadius: 999, background: label.bg, color: label.color,
        }}>{label.text}</span>
        {current.timingLabel && (
