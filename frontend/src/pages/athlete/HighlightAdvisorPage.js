  ...
  return (
    <div className="rounded-2xl border border-[var(--cm-border)] bg-[var(--cm-surface)] overflow-hidden" data-testid={testId}>
      <button onClick={() => setOpen(!open)}
        ...
        onMouseEnter={e => e.currentTarget.style.backgroundColor = "var(--cm-surface-2)"}
        onMouseLeave={e => e.currentTarget.style.backgroundColor = "transparent"}>
        <Icon className="w-4 h-4 flex-shrink-0" style={{ color }} />
        <span className="text-[12px] font-bold uppercase tracking-[0.1em] flex-1" style={{ color }}>{title}</span>
        <ChevronDown className={`w-3.5 h-3.5 text-[var(--cm-text)]/30 transition-transform ${open ? "" : "-rotate-90"}`} />
      </button>
...
export default function HighlightAdvisorPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto" data-testid="highlight-advisor-page">
      <div className="mb-6">
        <h1 className="text-lg font-bold text-[var(--cm-text)]">Highlight Reel Advisor</h1>
        <p className="text-[12px] text-[var(--cm-text)]/30">AI-powered advice for creating the perfect recruiting video</p>
      </div>
      ...
      <div className="rounded-2xl border border-[var(--cm-border)] bg-[var(--cm-surface)] p-5 mb-5" data-testid="highlight-input-section">
        <div className="flex items-end gap-2 mb-3">
          <textarea value={question} onChange={e => setQuestion(e.target.value)}
            ...
            className="flex-1 rounded-xl p-3 text-[13px] text-[var(--cm-text)] outline-none resize-none placeholder:text-[var(--cm-text)]/25 border"
          ...
          <button onClick={() => fetchAdvice(question)} disabled={loading}
            className="px-5 py-[11px] rounded-xl text-[12px] font-bold text-[var(--cm-text)] bg-[#1a8a80] disabled:opacity-40 inline-flex items-center gap-1.5"
        ...
        <div className="space-y-4">
          ...
          {advice.video_length && (
            <div className="flex items-center gap-3 rounded-2xl border border-[#1a8a80]/15 bg-[#1a8a80]/5 p-4" data-testid="video-length-card">
              ...
              <div>
                ...
                <div className="text-[14px] font-bold text-[var(--cm-text)]">{advice.video_length}</div>
              </div>
            </div>
          )}
                  ...
                  <div key={i} className="flex items-start gap-3 py-2 border-b border-white/[0.04] last:border-0">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <span className="text-[13px] font-semibold text-[var(--cm-text)]">{s.section}</span>
                        {s.duration && <span className="text-[11px] text-[var(--cm-text)]/30">{s.duration}</span>}
                      </div>
                      <p className="text-[12px] text-[var(--cm-text)]/40 mt-0.5">{s.description}</p>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <Section icon={AlertTriangle} title="Common Mistakes to Avoid" color="#ef4444" testId="avoid-section">
              <ul className="space-y-1.5">
                {advice.avoid.map((a, i) => (
                  <li key={i} className="flex items-start gap-2 text-[12px] text-[var(--cm-text)]/50">
          ...
          {advice.position_specific && (
            <Section icon={Target} title="Position-Specific Advice" color="#1a8a80" testId="position-section">
              <p className="text-[13px] text-[var(--cm-text)]/60 leading-relaxed">{advice.position_specific}</p>
            </Section>
          )}
          ...
          {advice.coach_perspective && (
            <Section icon={Video} title="What Coaches Look For" color="#a855f7" testId="coach-perspective-section">
              <p className="text-[13px] text-[var(--cm-text)]/60 leading-relaxed">{advice.coach_perspective}</p>
            </Section>
          )}
