    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          ...
          {primaryAction}
          <span style={{ fontWeight: 500, color: 'var(--cm-text-3, #94a3b8)', marginLeft: 5 }}>— {short}</span>
        </div>
    ...
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{
          ...
          {primaryAction}
          <span style={{ fontWeight: 400, color: 'var(--cm-text-4, #cbd5e1)', marginLeft: 5 }}>— {short}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 1 }}>
          <span style={{ width: 4, height: 4, borderRadius: '50%', background: '#10b981', opacity: 0.45, flexShrink: 0 }} />
          <span style={{ fontSize: 10, fontWeight: 600, color: 'rgba(16,185,129,0.45)' }}>On track</span>
        </div>
      </div>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }} data-testid="priority-board">
      ...
      <div data-testid="priority-section-attention">
        ...
        {high.length > 0 ? (
          ...
          <div style={{ padding: '14px 4px', fontSize: 12, color: 'var(--cm-text-3, #94a3b8)', fontWeight: 500 }} data-testid="empty-state-attention">
            Nothing urgent right now
          </div>
        )}
      </div>

      {/* ── Coming up — subtle tint ── */}
      <div data-testid="priority-section-coming-up" style={{ background: 'rgba(241,245,249,0.35)', borderRadius: 8, padding: '10px 8px 6px', margin: '0 -8px' }}>
        ...
        {medium.length > 0 ? (
          ...
          <div style={{ padding: '14px 4px', fontSize: 12, color: 'var(--cm-text-3, #94a3b8)', fontWeight: 500 }} data-testid="empty-state-coming-up">
            No upcoming actions
          </div>
        )}
      </div>
      ...
      <div data-testid="priority-section-on-track">
        <div
          ...
          style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: onTrackCollapsed ? 0 : 6, cursor: 'pointer', opacity: 0.7 }}
          data-testid="on-track-header"
        >
          <ChevronRight style={{
            width: 12, height: 12, color: '#10b981',
        ...
        {!onTrackCollapsed && (
          low.length > 0 ? (
            ...
            <div style={{ padding: '14px 4px', fontSize: 12, color: 'var(--cm-text-3, #94a3b8)', fontWeight: 500 }} data-testid="empty-state-on-track">
