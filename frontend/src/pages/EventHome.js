  ...
  return (
    ...
    >
      <div className="px-3.5 py-3 sm:px-5 sm:py-3.5">
        {/* Row 1: Name + timing + CTA */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-2.5">
          <div className="flex items-center gap-2 min-w-0">
            <h3 className="font-semibold text-slate-900 text-sm sm:truncate">{event.name}</h3>
            <span className={`text-[10px] font-bold tracking-wider px-1.5 py-0.5 rounded shrink-0 ${
          ...
          <button
            ...
            className={`flex items-center justify-center gap-1.5 w-full sm:w-auto px-3.5 py-2 sm:py-1.5 text-xs font-semibold text-white rounded-lg transition-colors shrink-0 ${cta.color}`}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1.5 sm:gap-0 mb-3">
          <div className="flex items-center gap-3 sm:gap-4 text-xs text-slate-500">
            <span className="flex items-center gap-1"><MapPin className="w-3 h-3 text-slate-400 shrink-0" />{event.location}</span>
            <span className="flex items-center gap-1"><GraduationCap className="w-3 h-3 text-slate-400 shrink-0" />{event.expectedSchools} schools</span>
          </div>
          {/* Athlete photo stack */}
              ...
              <div className="flex -space-x-2">
                {event.athlete_photos.slice(0, 5).map((a) => (
                  a.photo_url ? (
                    ...
                    <div
                      key={a.id}
                      className="w-6 h-6 sm:w-7 sm:h-7 rounded-full bg-gray-200 border-2 border-white flex items-center justify-center text-[8px] sm:text-[9px] font-bold text-gray-500"
                      title={a.name}
                    >
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
          {/* Blockers */}
          {hasBlockers && (
            <span className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md text-[10px] sm:text-[11px] font-medium bg-red-50 text-red-700 border border-red-100" data-testid={`event-blockers-${event.id}`}>
          ...
          {/* Needs attention */}
          {needsAttention && (
            <span className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md text-[10px] sm:text-[11px] font-medium bg-amber-50 text-amber-700 border border-amber-100">
              <Clock className="w-3 h-3" />
              <span className="hidden sm:inline">{readiness.needs_attention} need{readiness.needs_attention > 1 ? "" : "s"} attention</span>
              <span className="sm:hidden">{readiness.needs_attention} attention</span>
            </span>
          )}

          {/* Ready athletes */}
          {!isPast && readiness.ready > 0 && (
            <span className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md text-[10px] sm:text-[11px] font-medium bg-emerald-50 text-emerald-700 border border-emerald-100">
          ...
          {!isPast && (
            <span className="flex items-center gap-1.5 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md text-[10px] sm:text-[11px] font-medium bg-slate-50 text-slate-600 border border-slate-100">
              <div className="w-8 sm:w-10 h-1 rounded-full bg-slate-200 overflow-hidden">
          ...
          {isPast && event.capturedNotesCount > 0 && (
            <span className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md text-[10px] sm:text-[11px] font-medium bg-slate-50 text-slate-600 border border-slate-100">
              <FileText className="w-3 h-3" />
              {event.capturedNotesCount} notes
            </span>
          )}
          {isPast && event.followUpsPending > 0 && (
            <span className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md text-[10px] sm:text-[11px] font-medium bg-amber-50 text-amber-700 border border-amber-100" data-testid={`event-followups-${event.id}`}>
              <Clock className="w-3 h-3" />
              {event.followUpsPending} follow-up{event.followUpsPending > 1 ? "s" : ""}
            </span>
          )}
          {needsDebrief && (
            <span className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded-md text-[10px] sm:text-[11px] font-medium bg-amber-50 text-amber-700 border border-amber-100">
    ...
    <div data-testid="event-home-page">
      <div className="flex items-start sm:items-center justify-between gap-3 mb-5 sm:mb-6">
        <div className="min-w-0">
          <h1 className="text-base sm:text-lg font-semibold text-gray-900" data-testid="events-title">Events</h1>
          <p className="text-[11px] sm:text-xs text-gray-500 mt-0.5">Capture recruiting moments. Organize later.</p>
        </div>
        <button
          onClick={() => setShowCreate(true)}
          className="flex items-center gap-1.5 px-3 sm:px-4 py-2 text-xs sm:text-sm font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded-lg transition-colors shrink-0"
      ...
      <div className="flex items-center justify-between gap-2 mb-4 sm:mb-5">
        ...
        <select
          ...
          className="text-[11px] sm:text-xs border border-gray-200 rounded-md px-2 py-1.5 bg-white text-gray-700"
        >
