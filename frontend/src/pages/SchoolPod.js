...
const STRENGTH_CONFIG = {
  cold:   { label: "Cold",   color: "#94a3b8", bg: "rgba(148,163,184,0.1)" },
...
function RelationshipContext({ relationship }) {
  if (!relationship) return null;
  const { strength, interactions } = relationship;
  const cfg = STRENGTH_CONFIG[strength] || STRENGTH_CONFIG.cold;
  return (
    <div className="rounded-xl border px-4 py-3" style={{ backgroundColor: "var(--cm-surface, white)", borderColor: "var(--cm-border, #e2e8f0)" }} data-testid="relationship-context">
      <div className="flex items-center justify-between mb-2.5">
        <p className="text-[10px] font-bold uppercase tracking-wider" style={{ color: "var(--cm-text-3)" }}>Relationship Strength</p>
        <span className="text-[10px] font-bold uppercase px-2 py-0.5 rounded-full" style={{ backgroundColor: cfg.bg, color: cfg.color }}>
          {cfg.label}
        </span>
      </div>
      <div className="flex items-center gap-4 text-[11px]" style={{ color: "var(--cm-text-2, #64748b)" }}>
        <div className="flex items-center gap-1.5">
          <Mail className="w-3 h-3" />
          <span><strong>{interactions.emails}</strong> email{interactions.emails !== 1 ? "s" : ""}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <Phone className="w-3 h-3" />
          <span><strong>{interactions.calls}</strong> call{interactions.calls !== 1 ? "s" : ""}</span>
        </div>
        <div className="flex items-center gap-1.5">
          <Activity className="w-3 h-3" />
          <span><strong>{interactions.events}</strong> event{interactions.events !== 1 ? "s" : ""}</span>
        </div>
      </div>
    </div>
  );
