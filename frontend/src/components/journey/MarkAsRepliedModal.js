...
export function MarkAsRepliedModal({ programId, onSaved, onCancel }) {
  ...
  const save = async () => {
    ...
    setSaving(true);
    try { await axios.post(`${API}/athlete/programs/${programId}/mark-replied`, { note: note.trim() }); toast.success("Coach reply logged to timeline"); onSaved(); }
    catch { toast.error("Failed to log reply"); } finally { setSaving(false); }
  };
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" style={{ background: "rgba(0,0,0,0.6)", backdropFilter: "blur(12px)" }} data-testid="mark-replied-overlay">
