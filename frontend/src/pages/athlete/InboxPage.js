...
function ComposeModal({ open, onClose, replyTo, threadId, messageId, onSent }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" data-testid="compose-modal">
      <div className="w-full max-w-2xl mx-4 rounded-2xl bg-[#1a1f2e] border border-white/10 shadow-2xl">
        <div className="flex items-center justify-between px-5 py-3.5 border-b border-white/10">
          <h3 className="text-[14px] font-bold text-white">{replyTo ? "Reply" : "New Email"}</h3>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-white/5"><X className="w-4 h-4 text-white/40" /></button>
        </div>
        <div className="p-5 space-y-3">
          {!replyTo && (
            <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2">
              <span className="text-[11px] font-semibold text-white/30 w-8">To:</span>
              <input value={to} onChange={e => setTo(e.target.value)} placeholder="coach@university.edu"
                className="flex-1 bg-transparent text-[13px] text-white outline-none placeholder:text-white/20" data-testid="compose-to" />
            </div>
          )}
          <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2">
            <span className="text-[11px] font-semibold text-white/30 w-14">Subject:</span>
            <input value={subject} onChange={e => setSubject(e.target.value)} placeholder="Email subject"
              className="flex-1 bg-transparent text-[13px] text-white outline-none placeholder:text-white/20" data-testid="compose-subject" />
          </div>
          ...
          {!replyTo && (
            <div className="flex items-center gap-2 p-3 rounded-xl border border-[#1a8a80]/20 bg-[#1a8a80]/5">
              ...
              <select value={draftProgramId} onChange={e => setDraftProgramId(e.target.value)}
                className="flex-1 bg-transparent text-[12px] text-white/60 outline-none border-none" data-testid="draft-program-select">
          ...
          <textarea value={body} onChange={e => setBody(e.target.value)} placeholder="Write your message..." rows={10}
            className="w-full bg-white/[0.03] border border-white/10 rounded-xl p-3 text-[13px] text-white outline-none resize-none placeholder:text-white/20"
            data-testid="compose-body" />
        </div>
        <div className="flex items-center justify-end gap-2 px-5 py-3 border-t border-white/10">
          <button onClick={onClose} className="px-4 py-2 rounded-lg text-[12px] font-semibold text-white/40 border border-white/10">Cancel</button>
          <button onClick={handleSend} disabled={sending || !to || !body}
            className="px-5 py-2 rounded-lg text-[12px] font-bold text-white bg-[#1a8a80] disabled:opacity-40 inline-flex items-center gap-1.5"
  ...
  return (
    <div className="flex flex-col h-full" data-testid="thread-view">
      <div className="flex items-center gap-3 px-4 py-3 border-b border-white/10">
        <button onClick={onBack} className="p-1.5 rounded-lg hover:bg-white/5" data-testid="thread-back-btn">
          <ChevronLeft className="w-4 h-4 text-white/40" />
        </button>
        <div className="flex-1 min-w-0">
          <h3 className="text-[14px] font-bold text-white truncate">{thread.subject}</h3>
          <p className="text-[11px] text-white/30">{thread.messages?.length || 0} messages in thread</p>
        </div>
        <button onClick={() => onReply(thread.messages?.[thread.messages.length - 1])}
          className="px-3 py-1.5 rounded-lg text-[11px] font-bold bg-[#1a8a80] text-white inline-flex items-center gap-1" data-testid="thread-reply-btn">
          <Reply className="w-3.5 h-3.5" /> Reply
        </button>
      </div>
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        {(thread.messages || []).map((msg, i) => (
          <div key={msg.id} className="rounded-xl border border-white/[0.06] bg-[#1a1f2e] p-4" data-testid={`thread-msg-${i}`}>
            <div className="flex items-center justify-between mb-2">
              <div className="text-[12px] font-semibold text-white">{msg.from}</div>
              <div className="text-[10px] text-white/25">{msg.date}</div>
            </div>
            {msg.body_html ? (
              <div className="text-[13px] text-white/70 leading-relaxed prose prose-invert prose-sm max-w-none"
                dangerouslySetInnerHTML={{ __html: msg.body_html }} />
            ) : (
              <p className="text-[13px] text-white/70 leading-relaxed whitespace-pre-wrap">{msg.body_text || msg.snippet || ""}</p>
            )}
          </div>
        ))}
      </div>
  ...
  if (gmailConnected === false) {
    return (
      <div className="max-w-xl mx-auto text-center py-20" data-testid="inbox-not-connected">
        ...
        <h2 className="text-xl font-bold text-white mb-2">Connect Your Gmail</h2>
        ...
        <button onClick={connectGmail}
          className="px-6 py-3 rounded-xl text-[14px] font-bold text-white inline-flex items-center gap-2"
  ...
  return (
    <div className="max-w-4xl mx-auto" data-testid="inbox-page">
      ...
      <div className="flex items-center justify-between mb-5">
        <div>
          <h1 className="text-lg font-bold text-white" data-testid="inbox-title">Inbox</h1>
          <p className="text-[12px] text-white/30">Recruiting emails from .edu addresses & coaches</p>
        </div>
        <div className="flex gap-2">
          <button onClick={handleRefresh} disabled={refreshing}
            className="px-3 py-2 rounded-xl text-[12px] font-semibold text-white/50 bg-[#1a1f2e] border border-white/[0.06] inline-flex items-center gap-1.5"
            data-testid="inbox-refresh-btn">
            <RefreshCw className={`w-3.5 h-3.5 ${refreshing ? "animate-spin" : ""}`} /> Refresh
          </button>
          <button onClick={() => { setReplyContext(null); setShowCompose(true); }}
            className="px-4 py-2 rounded-xl text-[12px] font-bold text-white inline-flex items-center gap-1.5"
            style={{ background: "linear-gradient(135deg, #1a8a80, #25a99e)" }}
      ...
      <div className="flex gap-2 mb-5">
        <div className="flex-1 flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-[#1a1f2e] border border-white/[0.06]">
          <Search className="w-4 h-4 text-white/30" />
          <input value={searchQuery} onChange={e => setSearchQuery(e.target.value)}
            onKeyDown={e => e.key === "Enter" && handleSearch()}
            placeholder="Search emails..." className="flex-1 bg-transparent text-[13px] text-white outline-none placeholder:text-white/25"
      ...
      {emails.length === 0 ? (
        <div className="text-center py-16" data-testid="inbox-empty">
          <InboxIcon className="w-10 h-10 mx-auto mb-3 text-white/15" />
          <p className="text-sm text-white/30">No recruiting emails found</p>
          <p className="text-[12px] text-white/20 mt-1">Try sending an email to a coach first</p>
        </div>
      ) : (
        <div className="space-y-1" data-testid="email-list">
          {emails.map(em => (
            <div key={em.id} onClick={() => openThread(em.thread_id)}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl cursor-pointer transition-all hover:bg-white/[0.04] ${em.is_unread ? "bg-[#1a8a80]/5 border border-[#1a8a80]/10" : "border border-transparent"}`}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-0.5">
                  <span className={`text-[12px] font-semibold truncate ${em.is_unread ? "text-white" : "text-white/70"}`}>
                ...
                <div className={`text-[12px] truncate ${em.is_unread ? "font-semibold text-white/80" : "text-white/50"}`}>
                  {em.subject}
                </div>
                <div className="text-[11px] text-white/25 truncate mt-0.5">{em.snippet}</div>
              </div>
              <div className="flex-shrink-0 text-right">
                <div className="text-[10px] text-white/25">{em.date?.split(",")?.pop()?.trim()?.split(" ").slice(0, 3).join(" ")}</div>
