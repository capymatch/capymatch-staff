...
function TaskItem({ action, onComplete, onUpdate, athleteId, programId }) {
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 px-1 group relative" data-testid={`task-${action.id}`}>
      {/* Checkbox */}
      <button
      ...
      {action.due_date && !editing && (
        <span className="text-[10px] shrink-0" style={{ color: isOverdue ? "#dc2626" : "var(--cm-text-3, #94a3b8)" }}>
          {new Date(action.due_date).toLocaleDateString("en-US", { month: "short", day: "numeric" })}
        </span>
      )}

      {/* Visible nudge CTA for overdue athlete-owned tasks */}
      {isOpen && isOverdue && isAthlete && !editing && (
        <button
          onClick={handleNudge}
          disabled={nudging}
          className="shrink-0 inline-flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-semibold transition-colors hover:opacity-90 disabled:opacity-50"
