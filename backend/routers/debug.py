...
async def get_all_interventions(current_user: dict = get_current_user_dep()):
    ...
    return {
        "total_interventions": len(get_interventions()),
        "by_category": {
            "momentum_drop": len([i for i in get_interventions() if i['category'] == 'momentum_drop']),
            "blocker": len([i for i in get_interventions() if i['category'] == 'blocker']),
            "deadline_proximity": len([i for i in get_interventions() if i['category'] == 'deadline_proximity']),
            "engagement_drop": len([i for i in get_interventions() if i['category'] == 'engagement_drop']),
            "ownership_gap": len([i for i in get_interventions() if i['category'] == 'ownership_gap']),
            "readiness_issue": len([i for i in get_interventions() if i['category'] == 'readiness_issue']),
            "event_follow_up": len([i for i in get_interventions() if i['category'] == 'event_follow_up']),
        },
        "by_tier": {
            "critical": len([i for i in get_interventions() if i['priority_tier'] == 'critical']),
            "high": len([i for i in get_interventions() if i['priority_tier'] == 'high']),
            "medium": len([i for i in get_interventions() if i['priority_tier'] == 'medium']),
            "low": len([i for i in get_interventions() if i['priority_tier'] == 'low']),
        },
...
async def debug_athlete_interventions(athlete_id: str, current_user: dict = get_current_user_dep()):
    ...
    athlete_interventions = [i for i in get_interventions() if i['athlete_id'] == athlete_id]
...
async def get_intervention_scoring(intervention_id: str, current_user: dict = get_current_user_dep()):
    ...
    for intervention in get_interventions():
