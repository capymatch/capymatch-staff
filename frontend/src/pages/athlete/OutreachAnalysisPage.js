    ...
    <div className="flex flex-col items-center">
      <div className="relative w-[120px] h-[120px]">
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="text-[9px] font-semibold uppercase tracking-[1px] text-white/35 mt-1">{label}</span>
        </div>
      </div>
    </div>
  );
  ...
  if (!analysis) {
    return (
      <div className="max-w-2xl mx-auto text-center py-20" data-testid="outreach-empty">
        ...
        <p className="text-sm text-white/40">Add schools to your pipeline to see outreach analysis.</p>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-4xl mx-auto" data-testid="outreach-analysis-page">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-lg font-bold text-white">Outreach Analysis</h1>
          <p className="text-[12px] text-white/30">AI-powered analysis of your recruiting outreach</p>
        </div>
        <button onClick={fetchAnalysis} className="px-3 py-2 rounded-xl text-[12px] font-semibold text-white/50 bg-[#1a1f2e] border border-white/[0.06] inline-flex items-center gap-1.5"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-6">
        <div className="sm:col-span-1 flex justify-center items-center rounded-2xl border border-white/[0.06] bg-[#1a1f2e] p-6">
          {ai.overall_score != null ? (
            <ScoreRing score={ai.overall_score} label={ai.score_label || "Score"} />
          ) : (
            <div className="text-center text-white/30 text-xs">Analyzing...</div>
          )}
        </div>
        <div className="sm:col-span-3 grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            <div key={s.label} className="rounded-xl border border-white/[0.06] bg-[#1a1f2e] p-4" data-testid={`stat-${s.label.toLowerCase().replace(/\s+/g, '-')}`}>
              <s.icon className="w-4 h-4 text-[#1a8a80] mb-2" />
              <div className="text-xl font-extrabold text-white">{s.value ?? 0}</div>
              <div className="text-[10px] text-white/30 font-semibold uppercase tracking-wide">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* AI Summary */}
      {ai.summary && (
        <div className="rounded-2xl border border-white/[0.06] bg-[#1a1f2e] p-5 mb-5" data-testid="ai-summary">
          <p className="text-[13px] text-white/60 leading-relaxed">{ai.summary}</p>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-5">
        ...
        {ai.strengths?.length > 0 && (
          <div className="rounded-2xl border border-white/[0.06] bg-[#1a1f2e] p-5" data-testid="strengths-section">
        ...
        {ai.improvements?.length > 0 && (
          <div className="rounded-2xl border border-white/[0.06] bg-[#1a1f2e] p-5" data-testid="improvements-section">
      ...
      {ai.division_insights && (
        <div className="rounded-2xl border border-white/[0.06] bg-[#1a1f2e] p-5 mt-5" data-testid="division-insights">
          <h3 className="text-[12px] font-bold uppercase tracking-[0.1em] text-white/40 mb-2">Division Insights</h3>
          <p className="text-[13px] text-white/60 leading-relaxed">{ai.division_insights}</p>
        </div>
      )}
    </div>
  );
